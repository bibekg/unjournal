...
const TypePanelWrapper = styled.div`
  ...
  flex-basis: 0;
  textarea {
    color: ${props => props.theme.editor.text};
    ::placeholder {
      color: ${props => props.theme.editor.placeholderText};
    }
  }
...
export default class WritingView extends React.Component<PropsType> {
  ...
  endWriting() {
    this.setState({
      ...
      showHUD: true,
      state:
        this.typePanel && this.typePanel.value.length === 0 ? 'done' : 'done',
    })
  ...
  updateRemainingTime = () => {
    if (this.state.remainingTime <= 0) {
      ...
      const remainingTime =
        this.state.totalWriteTime - (Date.now() - this.state.startTime)
  ...
  render() {
    return (
      <WritingViewWrapper
        showHUD={this.state.showHUD || this.state.remainingTime === 0}
      >
        <TypePanelWrapper>
          {this.props.writingPrompt && [
            <WritingPrompt key={1}>
              <Text color={colors.brown} bold center size={18}>
                {this.props.writingPrompt}
              </Text>
            </WritingPrompt>,
            <Break key={2} />,
          ...
          <TypePanel
            innerRef={(ta: HTMLTextAreaElement) => {
              this.typePanel = ta
            }}
